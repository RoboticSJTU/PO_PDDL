...
def lint_texts(
    ...
    if cfg.treat_warnings_as_errors and result.warnings:
        promoted = [diagnostic for diagnostic in result.diagnostics if diagnostic.severity == "warning"]
...
def _build_symbol_table(
    ...
    return _SymbolTable(
        ...
        predicates={
            predicate.name: parsed_domain.predicate_parameter_types[predicate] for predicate in parsed_domain.predicates
        },
        observables={
            observable.name: parsed_domain.observable_parameter_types[observable]
...
def _check_reward_metric_consistency(
    ...
    if len(updated_functions) > 1:
        result.add(
            ...
            ("Action effects update multiple numeric functions: " + ", ".join(sorted(updated_functions)) + "."),
            "reward_metric",
            "domain",
        )
    ...
    if metric_target is not None and len(updated_functions) == 1:
        ...
        if metric_target != updated_name:
            result.add(
                ...
                (f"Problem metric uses `{metric_target}`, but action effects update `{updated_name}`."),
                "reward_metric",
                "problem",
                metric_target,
            )
...
def _check_default_policy_action_legality(
    ...
    substitution = {param_name: arg for (param_name, _type_name), arg in zip(action_schema.parameter_types, args)}
    ...
    for literal in sorted(required_true & policy_false):
        conflicts.append(f"action precondition requires `{literal}`, but policy precondition explicitly negates it")
    for literal in sorted(required_false & policy_true):
        conflicts.append(f"action precondition requires `not {literal}`, but policy precondition explicitly asserts it")
    if conflicts:
        result.add(
            "error",
            ...
            (
                ...
                f"`{action_schema.action.name}` under a conflicting precondition: " + "; ".join(conflicts) + "."
            ),
            "default_policy_action",
            "default_policy",
            rule_schema.rule.name,
