...
from ..data_structures.effect_bucket import ParsedEffectBucketAnnotation
...
from ..data_structures.type_node import TypeNode
...
def _parse_domain_raw(text: str) -> ParsedDomain:
    ...
    predicates, predicate_parameter_types = _parse_predicate_like_section(
        _find_section(root, ":predicates"), Predicate
    )
...
def _parse_predicate_like_section(
    ...
    ] | dict[Observable, list[tuple[str, str]]] = {}
    for item in section[1:]:
        if not isinstance(item, list) or not item:
        ...
        if comment_match is not None:
            ...
            current_annotations.append(
                ParsedEffectBucketAnnotation(
                    ...
                    success=(
                        None
                        if success_literal is None
                        else success_literal.strip().lower() == "true"
                    ),
...
def _parse_typed_symbol_sequence(
    tokens: list[SExpr] | list[str], *, default_type: str
) -> list[tuple[str, str]]:
