...
class FactorizedBelief:
    ...
    def validate(self, *, tolerance: float = 1e-3) -> None:
        ...
        if true_set & false_set:
            raise ValueError(
                "FactorizedBelief contains predicates that appear in both known_true and known_false."
            )
        ...
        for factor in self.factors:
            ...
            if factor_scope & true_set:
                raise ValueError(
                    f"Belief factor `{factor.name}` overlaps with known_true predicates."
                )
            if factor_scope & false_set:
                raise ValueError(
                    f"Belief factor `{factor.name}` overlaps with known_false predicates."
                )
            if factor_scope & covered:
                raise ValueError(
                    f"Belief factor `{factor.name}` overlaps with another factor scope."
                )
    ...
    def to_json(
        ...
        *,
        ...
        if grounded_predicates is None and predicate_index is None:
            raise ValueError(
                "FactorizedBelief.to_json requires either grounded_predicates or predicate_index."
            )
        if grounded_predicates is not None and predicate_index is not None:
            raise ValueError(
                "FactorizedBelief.to_json accepts grounded_predicates or predicate_index, not both."
            )
        ...
        if grounded_predicates is not None:
            predicate_index = {
                predicate: index for index, predicate in enumerate(grounded_predicates)
            }
