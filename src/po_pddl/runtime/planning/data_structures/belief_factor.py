...
class BeliefFactor:
    ...
    def validate(self, *, tolerance: float = 1e-3) -> None:
        ...
        for probability, true_predicates in self.cases:
            if probability < 0.0:
                raise ValueError(
                    f"Belief factor `{self.name}` contains a negative probability."
                )
            true_set = set(true_predicates)
            if len(true_set) != len(true_predicates):
                raise ValueError(
                    f"Belief factor `{self.name}` contains duplicate predicates in a case."
                )
            if not true_set.issubset(scope_set):
                raise ValueError(
                    f"Belief factor `{self.name}` contains a case predicate outside its scope."
                )
            prob_sum += probability

        if abs(prob_sum - 1.0) > tolerance:
            raise ValueError(
                f"Belief factor `{self.name}` probabilities sum to {prob_sum}, not 1.0."
            )
